...
public:
    struct Option
    {
        ...
        double fx() { return camera_info[0]; }
        double fy() { return camera_info[1]; }
        double cx() { return camera_info[2]; }
        double cy() { return camera_info[3]; }

        Option(const uint& _max_iters = 100, const double& _eps = 1e-6, const Eigen::Matrix<double, 4, 1>& _camera_info = Eigen::Matrix<double, 4, 1>::Identity())
